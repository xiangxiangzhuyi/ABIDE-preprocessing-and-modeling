% After the coregister, it is necessary to check the result manually. The
% GUI "Check Reg" tool of SPM12 is very, but it is not very efficient to
% check so many subjects, so I decide to use the check function to do this
% work. In order to check fMRI images as many as possible, I will randomly
% choose 3 images (include  one mean imgae) from the whole sequence.
...
for i=1:1000
    if check_result(index).coregister~=0
        index=index+1;
        continue;
    end 
    ...
    str='spm_check_registration(anat_filename, func_mean_filename ';
    ...
    if check_num==-1
        ...
        check_result(index).coregister=check_num;
        index=index+1;
    end
